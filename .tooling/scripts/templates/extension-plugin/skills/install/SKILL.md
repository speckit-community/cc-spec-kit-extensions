---
name: "install"
description: "Install the {EXT_NAME} extension into your project. Copies runtime files, registers hooks, and makes commands available."
...
Install the **{EXT_NAME}** extension into the current project. This copies pre-generated runtime files, registers lifecycle hooks, updates the extension registry, and makes the extension's commands available as skills.
...
cp -R "${CLAUDE_PLUGIN_ROOT}/assets/bash/.specify/extensions/{EXT_ID}" \
      .specify/extensions/{EXT_ID}

# Copy command skills
cp -R "${CLAUDE_PLUGIN_ROOT}/assets/bash/.claude/skills"/speckit-{EXT_ID}-* \
      .claude/skills/
```
...
```powershell
Copy-Item -Recurse -Force "${CLAUDE_PLUGIN_ROOT}/assets/ps/.specify/extensions/{EXT_ID}" .specify/extensions/{EXT_ID}
Copy-Item -Recurse -Force "${CLAUDE_PLUGIN_ROOT}/assets/ps/.claude/skills/speckit-{EXT_ID}-*" .claude/skills/
```
...
```bash
find .specify/extensions/{EXT_ID} -name '*.sh' -exec chmod +x {} +
```
...
For each hook:
- If **not present** at the hook point → append the entry with fields: `extension: {EXT_ID}`, `command`, `enabled: true`, `optional`, `prompt`, `description`, `condition: null`
...
```bash
shasum -a 256 .specify/extensions/{EXT_ID}/extension.yml | cut -d' ' -f1
```
...
```
✅ Extension "{EXT_NAME}" installed successfully!
